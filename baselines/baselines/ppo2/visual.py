...
def plot_obs(obs, obs_list = None):
    if obs_list is None:
        ...
        if obs_list.shape[0]<100:
            obs_list = np.vstack((obs_list,obs[0,14:17]))
        else:
            obs_list = np.roll(obs_list, -1, axis=0)
            obs_list[-1] =  obs[0,14:17]
...
def plot_3d_obs(obs, obs_list = None, done= None):
    if done[0]:
        obs_list = None
        return obs_list
    ...
    # print("obs_list[:,0]")
    # print(obs_list[:,0])
    ax.plot(obs_list[:,0],obs_list[:,1],obs_list[:,2],'-o', linewidth=2, color="blue")
